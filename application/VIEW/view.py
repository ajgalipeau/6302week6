...
class View(toga.App):
    """
    A class to represent the view layer of the application.

    Methods:
    -------
    startup():
        Initializes the application and sets up the main window and its contents.
    on_close_handler(widget):
        Handles the event when the main window is closed.
    button_handler(widget):
        Handles the event when the "Add User" button is pressed.
    """
    ...
    def startup(self):
        ...
        right_content.add(self.success_message)  # Add the message below the button

        # Create a box for displaying grade information
        key_content = toga.Box(style=Pack(direction=COLUMN, padding=10))
        grade_key = toga.Label("Grade Key: A=4.0, B=3.0, C=2.0, D=1.0, F=0.0, I=0.0", style=Pack(padding=5))
        key_content.add(grade_key)
        ...
        # Attach the on_close event handler
    ...
    def on_close_handler(self, widget):
        ...
        -------
        bool
            True to confirm the window can close.
        """
        # Basic handler for when the window is closed
        print("\nWindow is closing.\n")
        return True  # Return True to confirm the window can close
